...
echo -e "${BLUE}[INFO] 下载 XanMod GPG 公钥...${PLAIN}"

rm -f /tmp/xanmod.key /etc/apt/keyrings/xanmod-archive-keyring.gpg
...
if ! grep -q "BEGIN PGP PUBLIC KEY BLOCK" /tmp/xanmod.key; then
    echo -e "${RED}[错误] XanMod 公钥下载失败，返回内容不是 PGP KEY。${PLAIN}"
    echo -e "${YELLOW}可能原因：Cloudflare 防护拦截。${PLAIN}"
    exit 1
fi
...
if [ ! -s /etc/apt/keyrings/xanmod-archive-keyring.gpg ]; then
    echo -e "${RED}[错误] XanMod KEY 导入失败！${PLAIN}"
    exit 1
fi
...
if ! apt-cache show "$KERNEL_PKG" >/dev/null 2>&1; then
    echo -e "${YELLOW}[提示] 默认包 ${KERNEL_PKG} 不存在，正在搜索可用 XanMod 内核...${PLAIN}"

    AUTO_KERNEL=$(apt-cache search linux-xanmod | awk '{print $1}' | grep -E "linux-xanmod.*x64v3|linux-xanmod.*x64v4|linux-xanmod" | head -n 1)

    if [ -n "$AUTO_KERNEL" ]; then
        KERNEL_PKG="$AUTO_KERNEL"
        echo -e "${GREEN}[OK] 自动选择内核包: ${KERNEL_PKG}${PLAIN}"
    else
        echo -e "${RED}[错误] 未找到 XanMod 内核包，请检查软件源。${PLAIN}"
